 ...
 * black keys overlay at 62% width. Pressing a key swaps its fill, lights a
 * colored glow and depresses it 2px. Reports MIDI-ish note indices.
 */
export function Keyboard({
  ...
  const { white, black } = useMemo(() => {
    const white: { idx: number; style: CSSProperties }[] = [];
    for (let i = 0; i < N; i++) {
      white.push({
    ...
    const bw = ww * 0.62;
    const black: { idx: number; style: CSSProperties }[] = [];
    for (let i = 0; i < N - 1; i++) {
  ...
  const press =
    (pressCol: string, idx: number) => (e: React.PointerEvent<HTMLDivElement>) => {
  ...
  const release =
    (base: string, idx: number) => (e: React.PointerEvent<HTMLDivElement>) => {
      const el = e.currentTarget;
    ...
    >
      {white.map((w, i) => (
        <div
          key={"w" + i}
          style={w.style}
          onPointerDown={press(whitePress, w.idx)}
      ...
      {black.map((b, i) => (
        <div
          key={"b" + i}
          style={b.style}
          onPointerDown={press(blackPress, b.idx)}
